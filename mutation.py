...
class SimpleRandomMutation(Mutation[FloatSolution]):
    ...
    def execute(self, solution: FloatSolution) -> FloatSolution:
        ...
        for i in range(solution.number_of_variables):
            rand = random.random()  #Do not need
            if rand <= self.probability:
...
class ModifiedSimpleRandomMutation(Mutation[IntegerSolution]):

    def __init__(self, probability: float):
        super(ModifiedSimpleRandomMutation, self).__init__(probability=probability)
    ...
    def execute(self, solution: IntegerSolution) -> IntegerSolution:
        ...
        for i in range(solution.number_of_variables):
            rand = random.random()
            if rand <= self.probability:
                solution.variables[i] = randrange(solution.lower_bound[i],solution.upper_bound[i])
        return solution

    def get_name(self):
        return 'Modified Simple random_search mutation'
