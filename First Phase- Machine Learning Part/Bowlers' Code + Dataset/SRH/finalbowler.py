...
''' Importing Dataset '''
##################################
dataset=pd.read_csv('T_Natarajan.csv')
...
for value in df["SOver"]:
    if value>=1 and value<=9.7:
        OverForm.append(1)
    elif value>=10 and value<=24.7:
        OverForm.append(2)
    elif value>=25 and value<=49.7:
        OverForm.append(3)
...
Performance_df = pd.DataFrame({'Player_Name':['T_Natarajan'],'Total Wicket':[Total_Wicket],'Cost':[17]})
#Performance_df = pd.DataFrame(Total_Run, columns= ['Total Run'])

Performance_df.to_csv (r'T_Natarajan_Performance.csv', index = False, header=True)    
