...
''' Importing Dataset '''
##################################
dataset=pd.read_csv('Shardul Thakur.csv')
...
for value in df["OppOver"]:
    if value>=0 and value<=5.6:
        ...
        OverOpp.append(2)
    elif value>=11 and value<=35.6:
        OverOpp.append(3)
    elif value>=36 and value<=60.6:
        OverOpp.append(4)
    elif value>=61 :
        OverOpp.append(5)
df["OverOpp"] = OverOpp   
...
Performance_df = pd.DataFrame({'Player_Name':['Shardul Thakur'],'Total Wicket':[Total_Wicket],'Cost':[17]})
#Performance_df = pd.DataFrame(Total_Run, columns= ['Total Run'])

Performance_df.to_csv (r'Shardul Thakur_Performance.csv', index = False, header=True)    
