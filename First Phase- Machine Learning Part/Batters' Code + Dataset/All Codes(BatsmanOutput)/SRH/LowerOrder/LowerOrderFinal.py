...
''' Importing Dataset '''
##################################
dataset=pd.read_csv('Mohammad NabiBowling Allrounder.csv')
...
for value in df["CalAvg"]:
    if value>=0.0 and value<=7.99:
        AvgRating.append(1)
    elif value>=8.0 and value<=12.99:
       AvgRating.append(2)
    elif value>=13.0 and value<=18.99:
        AvgRating.append(3)
    elif value>=19.0:
        AvgRating.append(4)
...
Performance_df = pd.DataFrame({'Player_Name':['Mohammad Nabi'],'Total Run':[Total_Run],'Cost':[17]})
#Performance_df = pd.DataFrame(Total_Run, columns= ['Total Run'])

Performance_df.to_csv (r'Mohammad Nabi_Performance_Result.csv', index = False, header=True)    
