...
''' Importing Dataset '''
##################################
dataset=pd.read_csv('Shane Watson.csv')
...
for value in df["SRTotal"]:
    if value>=8.0 and value<=13.999999:
        SRCon.append(5)
...
Performance_df = pd.DataFrame({'Player_Name':['Shane Watson'],'Total Wicket':[Total_Wicket],'Cost':[17]})
#Performance_df = pd.DataFrame(Total_Run, columns= ['Total Run'])

Performance_df.to_csv (r'Shane Watson_Performance.csv', index = False, header=True)    
